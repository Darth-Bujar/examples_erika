...
#define MMC_CD      (!(uint8)IfxPort_getPinState(SD_CD))    /* Card detected   (yes:true, no:false, default:true) */
...
BYTE xchg_spi (BYTE dat){
    ...
    get_rx_buff(&result, 1);    /* Get received byte */
    return result;
}
...
{
    set_tx_buff((uint8*)buff, (uint16)cnt);
    qspiTransfer((uint16)cnt);
    while(!get_rdy());      /* Wait for end of the SPI transaction */
...
{
    clear_first_n_bytes_of_rx_buff((uint16)cnt);
    clear_first_n_bytes_of_tx_buff((uint16)cnt);
    qspiTransfer((uint16)cnt);
    while(!get_rdy());
    get_rx_buff((uint8*)buff, (uint16)cnt);
}
...
void disk_timerproc (void) // Treba dat do timeru ak chcete checkovat presence
{
    BYTE s;
    UINT n;
