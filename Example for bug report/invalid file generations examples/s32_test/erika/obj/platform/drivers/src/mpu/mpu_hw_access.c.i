# 1 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c"
# 1 "D:\\RTOS_PROJEKT\\erica\\workspace\\s32_test\\erika//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c"
# 37 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c"
...
# 38 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c" 2
# 71 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c"
void MPU_SetRegionAddr(MPU_Type * const base,
                       uint8_t regionNum,
                       uint32_t startAddr,
                       uint32_t endAddr)
                              ...
                              const mpu_master_access_right_t * masterAccRight)
{
    ...
    if (masterNum <= (3U))
    {
        ...
        accRight = (((uint32_t)(((uint32_t)(((uint32_t)masterAccRight->accessRight & (0x07U)) >> (0U)))<<0u))&0x7u)
                 | (((uint32_t)(((uint32_t)(((uint32_t)masterAccRight->accessRight & (0x18U)) >> (3U)))<<3u))&0x18u);
        accMask = 0x7u | 0x18u;


        if (masterNum < (2U))
        {
            accRight |= (((uint32_t)(((uint32_t)(masterAccRight->processIdentifierEnable ? 1UL : 0UL))<<5u))&0x20u);
        }
        accMask |= 0x20u;
        ...
        accShift = (masterNum * (6U));
    }
    else
    {

        accRight = (((uint32_t)(((uint32_t)(((uint32_t)masterAccRight->accessRight & (0x20U)) >> (5U)))<<24u))&0x1000000u)
                 | (((uint32_t)(((uint32_t)(((uint32_t)masterAccRight->accessRight & (0x40U)) >> (6U)))<<25u))&0x2000000u);
        accMask = 0x1000000u | 0x2000000u;

        accShift = (2U) * (masterNum - ((3U) + 1U));
    }
                    ...
                    uint8_t regionNum)
{
    ...
    if (regionNum == 0U)
    {
        ...
        base->RGDAAC[0U] = (0x0061F7DFU);
    }
    else
    {
        base->RGD[regionNum].WORD3 = 0U;
        base->RGD[regionNum].WORD2 = 0U;
        base->RGD[regionNum].WORD1 = (0x1FU);
...
# 184 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c"
    errInfoPtr->accessCtr = (uint16_t)((temp & 0xFFFF0000u) >> 16u);


    errInfoPtr->master = (uint8_t)((temp & 0xF0u) >> 4u);


    errInfoPtr->attributes = (mpu_err_attributes_t)((temp & 0xEu) >> 1u);


    errInfoPtr->accessType = (mpu_err_access_type_t)((temp & 0x1u) >> 0u);
    ...
    errInfoPtr->processorIdentification = (uint8_t)((temp & 0xFF00u) >> 8u);
                                  ...
                                  mpu_master_access_right_t * masterAccRight)
{
    ...
    if (masterNum <= (3U))
    {
        masterAccRight->accessRight = (MPU_SUPERVISOR_RWX_USER_RWX);
    }
    else
    {
        masterAccRight->accessRight = (MPU_RW);
    }


    masterAccRight->processIdentifierEnable = 
# 225 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c" 3 4
                                             0
# 225 "C:\\NXP\\S32_SD~1.3\\platform\\drivers\\src\\mpu\\mpu_hw_access.c"
                                                  ;
